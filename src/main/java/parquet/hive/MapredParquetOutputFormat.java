...
public class MapredParquetOutputFormat extends FileOutputFormat<Void, ArrayWritable> implements HiveOutputFormat<Void, ArrayWritable> {
    ...
    public RecordWriter<Void, ArrayWritable> getRecordWriter(final FileSystem ignored,
        ...
        final String columnNameProperty = job.get("columns");
        final String columnTypeProperty = job.get("columns.types");
        ...
        DataWritableWriteSupport.setSchema(HiveSchemaConverter.convert(columnNames, columnTypes), job);
        Path path =  new Path(getWorkOutputPath(job), name+ ".parquet");
        return new RecordWriterWrapper(realOutputFormat, job,
                path.toString(), progress);
    }
